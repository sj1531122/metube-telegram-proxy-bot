...
class MeTubeClientTests(IsolatedAsyncioTestCase):
    ...
    async def test_clear_done_entries_posts_expected_payload(self):
        calls = []
        ...
        result = await client.clear_done_entries(["done-1", "done-2"])
        ...
        self.assertEqual(
            ...
            [
                (
                    "https://metube.example/delete",
                    {"where": "done", "ids": ["done-1", "done-2"]},
                    {"Authorization": "Bearer token"},
                )
            ],
        )
    ...
    async def test_clear_done_entries_wraps_transport_error(self):
        client = MeTubeClient(base_url="https://metube.example", timeout_seconds=7)

        with patch.object(MeTubeClient, "_default_post_json", side_effect=URLError("delete down")):
            with self.assertRaisesRegex(MeTubeApiError, "delete down"):
                await client.clear_done_entries(["done-1"])
    ...
    async def test_clear_done_entries_wraps_json_decode_error(self):
        client = MeTubeClient(base_url="https://metube.example", timeout_seconds=7)
